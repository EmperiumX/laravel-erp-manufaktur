                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-4 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                        <div class="flex gap-2 flex-wrap">
                            <a href="{{ route('materials.create') }}" class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                                <i class="ri-add-line mr-1"></i> Tambah Bahan Baku
                            </a>
                            ...
                            <a href="{{ route('materials.export') }}" class="inline-block px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">
                                <i class="ri-file-excel-line mr-1"></i> Export Excel
                            </a>
                            <a href="{{ route('materials.template') }}" class="inline-block px-4 py-2 bg-teal-600 text-white rounded hover:bg-teal-700">
                                <i class="ri-download-2-line mr-1"></i> Download Template
                            </a>
                        </div>
                        <form action="{{ route('materials.import') }}" method="POST" enctype="multipart/form-data" class="flex items-center gap-2 p-2 border border-gray-200 rounded-lg bg-gray-50">
                            @csrf
                            ...
                            <tbody>
                                ...
                                <tr class="cursor-pointer hover:bg-blue-50 transition-colors">
                                    <td class="border border-gray-300 px-2 py-2 text-center">
                                        <input type="checkbox" name="ids[]" value="{{ $material->id }}" class="material-checkbox rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    </td>
                    ...
                    <form action="{{ route('materials.bulk-destroy') }}" method="POST" id="bulkDeleteForm" style="display:none;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data bahan baku yang terpilih?');">
                        @csrf
                    </form>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#checkAll').on('change', function() {
                ...
                $('input[type="checkbox"].material-checkbox', rows).prop('checked', this.checked);
                toggleBulkDeleteButton();
            });

            // Handle individual checkbox change
            $('#materialTable tbody').on('change', 'input[type="checkbox"].material-checkbox', function() {
                lastChecked = this;
                var rowsCount = table.rows({ 'search': 'applied' }).nodes().length;
                var checkedCount = 0;
                var rows = table.rows({ 'search': 'applied' }).nodes();
                
                $('input[type="checkbox"].material-checkbox', rows).each(function() {
                    if (this.checked) {
                        checkedCount++;
                    }
                });
            ...
            $('#materialTable tbody').on('click', 'tr', function(e) {
                if ($(e.target).is('a, button, input, select, textarea, i') || $(e.target).closest('a, button, input, select').length) {
                    return;
                }

                var checkbox = $(this).find('input[type="checkbox"].material-checkbox');
                ...
                if (e.shiftKey && lastChecked) {
                    var rows = table.rows({ 'search': 'applied' }).nodes();
                    var checkboxes = $('input[type="checkbox"].material-checkbox', rows);
